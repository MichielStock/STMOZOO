...
In this project, the Hungarian algorithm is implemented using an adjacency matrix.
...
This algorithm solves what can be also called the assignment problem. An example can be, given $$n$$ agents and the money they ask for performing each task, finding which agent performs which task in order to minimize the cost of performing all $$m$$ tasks.
When there are the same number of agents than tasks, the the problem is called balanced assignment, otherwise is called unbalanced assignment. And if the total cost is the sum for each agent performing its task, the problem is called linear assignment. This implementation can be used to solve linear balanced assignment problems.
"""
...
md"""
**`find_min_row`** is a function that gets the row with the fewest zeros. For that purpose, it needs as an input a binary matrix, having `0` if the original value after subtracting the smallest entry in each row from all the other entries in the row was `0`, and a `1` if it was different from `0`. It also needs a vector `zero_list` that will store the coordinates of the `0` in the row with fewest zeros. After the first finding, it reajusts the binary matrix so the whole row and column from the last coordinate stored is set to ones (`1`). This function is used in the next function `mark_matrix`.
"""

# ╔═╡ 8fa3a099-d325-4675-9c05-491f25ea73c8
"""
	find_min_row(matrix_zero, zero_list) 

Inputs:
	- `matrix_zero`: binary matrix
...
function find_min_row(matrix_zero, zero_list)
	...
	for row in 1:size(matrix_zero)[1]
		if 0 in matrix_zero[row, :] # Needs a 0 in the row to create a frequency dict
			# And if the number of zeros < than the last min stored
			if min_row[1] > countmap(matrix_zero[row,:])[0] 
				#stores the number of zeros and the row index
				min_row = [countmap(matrix_zero[row,:])[0], row]
			end
		end
	end
	# Get the column index 
	zero_index = findall(x->x==0, matrix_zero[min_row[2],:])[1]  
	...
	matrix_zero[min_row[2], :] .= 1
	matrix_zero[:, zero_index] .= 1
	return min_row, zero_list
end

# ╔═╡ 571d7a9b-0c6b-466e-9761-3f4a4ada9e86
md"""
**`mark_matrix`** needs as an input the matrix with the already subtracted the smallest entry in each row from all the other entries and it returns the `zero_list` and the marked row and column indexes. For that, it creates the binary matrix and calls the function `find_min_row`. It performs steps 2 and 3.
"""
...
function mark_matrix(mat)
	...
	#Transform the matrix to binary matrix(0 = 0, others = 1)
	binary_mat = copy(mat)
	binary_mat[binary_mat .!= 0] .= 1
	binary_mat_copy = copy(binary_mat)
	#Recording possible answer positions by zero_list
    zero_list = []	
    while 0 in binary_mat_copy
        find_min_row(binary_mat_copy, zero_list)
	end
	...
	# Get non marked rows
    non_marked_row = (x->collect(x))(setdiff(Set(1:size(mat)[1]), Set(zero_list_row)))
	...
	marked_rows = (x->collect(x))(setdiff(Set(1:size(mat)[1]), Set(non_marked_row)))   
...
Inputs:
	- `mat` : binary matrix created and modified by `mark_matrix`
...
function adjust_matrix(mat, marked_rows, marked_cols)
    ...
    for row in 1:size(adjusted)[1]
        if row ∉ marked_rows
            for i in 1:length(adjusted[row,:])
                if i ∉ marked_cols
...
# ╔═╡ 0dd294c1-1b4e-44fb-990d-1d12cfc067c4
md"""
...
# ╔═╡ 41b83fdc-8d80-4791-83aa-ad1c678728e1
"""
...
function Hungarian_algorithm(mat)
    ...
    while num_zeros < len
		...
		# Step 3: if marked rows & columns is less than len, go to step 4.
        if num_zeros < len
