...
md"""
![](https://github.com/CeriAnneLaureyssens/CrossEntropy.jl/blob/master/notebook/figures/CE_visual.jpg?raw=true)
"""
...
Now the CE method itself is implemented we will try it out on the importance sampling technique.
...
# Plot the base distribution and the optimized distribution
begin
	using Plots
	x = 1:5
	y = [0.74899, 0.2, 0.05, 0.001, 0.00001] # Base distribution
	plot(x, y, title = "Importance sampling", label = "Base distribution")
	
	z = is_dist_opt[:x][1].p # Optimized distribution
	plot!(x, z, label = "Optimal distribution")
end
